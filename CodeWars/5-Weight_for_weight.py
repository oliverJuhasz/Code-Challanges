...
When two numbers have the same "weight", let us class them as if they were strings and not numbers: 100 is before
180 because its "weight" (1) is less than the one of 180 (9) and 180 is before 90 since, having the same "weight" (9)
...
def order_weight(strng):
    weights_list = strng.split(" ")
    weights_list.sort(key = lambda x: (sum([int(num) for num in x]), x))
